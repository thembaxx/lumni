...
- **DOM API in tests**: Prefer `getElementsByTagName` / `getElementsByClassName` + `container.textContent` regex matching over `querySelector` / `querySelectorAll` (happy-dom `SelectorParser` throws `TypeError` on `new this.window.SyntaxError` in test setups).
...
- [2026-06-02] **Per-question RAG source persistence** (`f769f322`): `Question.webSources?: { url, title }[]` field added (Dexie v26, lazy rehydrate, no backfill); hybrid AI-cite + all-sources-fallback in `src/lib/question-engine/source-mapper.ts`; `PromptManager.appendSourceRefsAppendix()` instructs the model to return `sourceRefs: number[]` per question; new `<SourceAttributionPill>` rendered on `QuestionCardFeedback` (4th VerifiedByPill consumer, lighter inline pill).
...
- **happy-dom `querySelector` SyntaxError (Session 20-21)**: `screen.getByText` / `screen.queryByText` / `container.querySelector("a")` all throw `TypeError: undefined is not a constructor (evaluating 'new this.window.SyntaxError(...)')`. Workaround: use `container.textContent` regex matching AND `getElementsByTagName` / `getElementsByClassName` (DOM API, no selector parsing).
