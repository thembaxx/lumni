...
- [x] **High-severity catch sweep** (~40 files) — Added `logError()` to retention-loop/_, observability/events.ts, visual-engine/_, question-engine/_, share-service.ts, ai/_, ocr, hooks/\*, etc.
...
- [x] Add `loading.tsx` to tier-1 routes (auth/_, bookmarks, study-groups/_, search, review, premium) — uses shared `<PageSkeleton>`
...
> Manual follow-up status (2026-07-08): The P2 dark-mode contrast defect is already fixed in code (all 13 sites use `text-system-accent-foreground`; token present in both themes) — no fix needed. `pnpm run todo:sync` is BLOCKED in this environment: `LINEAR_API_KEY` is not authenticated (`FAIL ... not authenticated` on every issue). Run it from an environment with a valid `LINEAR_API_KEY` to push the 3 audit entries to Linear.
...
- [#84](https://github.com/thembaxx/lumni/issues/84) — **Set up branch protection rules** — GitHub.com → Settings → Branches → Add rules for `dev` (require status checks: quality, build, unit-tests; require review) and `master` (same + linear history, only PRs from dev)
  - [#85](https://github.com/thembaxx/lumni/issues/85) — **Raise test coverage thresholds** — DONE via plan 147: `coverage.thresholds` already exist in `vitest.config.ts` (statements 10, branches 5, functions 8, lines 10). Raising to the proposed 30%/20% is NOT safe — actual coverage is 23.28% stmts / 17.74% branches and would fail CI. Left at safe values.
  - [#86](https://github.com/thembaxx/lumni/issues/86) — **Fix context-sync.yml placeholder** — DONE via plan 142: workflow changed from daily `schedule` cron to `workflow_dispatch` (manual), dead script reference removed. No longer a no-op cron.
