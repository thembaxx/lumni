...
test.describe("Swipeable quiz navigation", () => {
	...
	test("swipe left navigates to next question", async ({ page }) => {
		...
		await page.waitForFunction(
			() => document.body.textContent?.includes("What is 2 + 2?"),
			{ timeout: 15000 },
		);
		...
		await page.waitForFunction(
			() => document.body.textContent?.includes("What is 3 + 3?"),
			{ timeout: 5000 },
		);

		await expect(
			page.locator("text=What is 3 + 3?").first(),
	...
	test("swipe right returns to previous question", async ({ page }) => {
		...
		await expect(
			page.locator("text=What is 2 + 2?").first(),
		).toBeVisible({ timeout: 3000 });
	});
...
test.describe("Pull-to-refresh", () => {
	test("pull gesture applies translateY transform", async ({ page }) => {
		...
		const transformAfterMove = await scrollContainer.evaluate(
		...
		const transformAfterEnd = await scrollContainer.evaluate(
			(el) => el.style.transform,
		);
