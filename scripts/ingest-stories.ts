...
async function ingestFromProjectGutenberg(): Promise<RawStory[]> {
	...
	const sources: { id: string; title: string; author: string; url: string; subjects: string[]; topics: string[] }[] = [
		{
...
async function writeStoryFiles(stories: RawStory[]): Promise<void> {
	for (const story of stories) {
		...
		if (fs.existsSync(metaIndexPath)) {
			...
			// Replace or append
			const existingMatch = indexContent.match(
				new RegExp(`id: "${story.id}"`),
			);
			if (!existingMatch) {
