...
async function main() {
  const endpoint = process.env.NEXT_PUBLIC_APPWRITE_ENDPOINT || "https://jnb.cloud.appwrite.io/v1";
  ...
  for (const [id, col] of Object.entries(report.collections)) {
    ...
    if ((col as Record<string, unknown>).attributesCreated) {
      parts.push(`${(col as Record<string, unknown>).attributesCreated} attrs`);
    }
    if ((col as Record<string, unknown>).indexesCreated) {
      parts.push(`${(col as Record<string, unknown>).indexesCreated} indexes`);
    }
  ...
  for (const [id, seeded] of Object.entries(report.seeded)) {
    console.log(`Seeded "${id}": ${seeded.inserted} inserted, ${seeded.skipped} skipped`);
