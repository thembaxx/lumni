...
export const MessageBubble = memo(function MessageBubble({ message, onRetry }: MessageBubbleProps) {
  ...
  if (message.type === "image") {
    ...
    const isPaperPlaneing =
      message.processingStatus === "sending" || message.processingStatus === "idle";
          ...
          >
            {message.imageUrl ? (
              ...
              >
                <SmartImage
                  ...
                  className={cn("max-h-50 w-full object-cover", isError && "opacity-50 grayscale")}
                />
                <div
                  className={cn(
            ...
            <div className="flex items-center justify-between gap-3 p-2 px-3">
              <span className="ios-caption-3 max-w-30 truncate font-extrabold uppercase tracking-tight opacity-80">
                ...
                {message.imageFileSize && ` (${formatBytes(message.imageFileSize)})`}
              </span>
              {isError && onRetry && (
                <Button
                  variant="ghost"
    ...
    return (
      ...
      >
        ...
        >
          <span
            className={cn(
              "absolute inset-0 flex items-center justify-center transition-[opacity,scale,filter] duration-300 ease-(--ease-ios-decelerate)",
              isPlaying ? "scale-100 opacity-100 blur-0" : "scale-[0.25] opacity-0 blur-sm",
            )}
          >
            <HugeiconsIcon icon={SquareIcon} className="size-3.5 fill-current" />
          </span>
          <span
            className={cn(
              "absolute inset-0 flex items-center justify-center transition-[opacity,scale,filter] duration-300 ease-(--ease-ios-decelerate)",
