...
export function OTPDialog({ open, onOpenChange, onSuccess }: AuthDialogProps) {
  ...
  const { email, otp, error, sent, countdown, remainingAttempts, verified } = form;
  ...
  const handleVerify = useCallback(async () => {
    ...
    try {
      ...
      localStorage.setItem("admin_access", data.isFullAccess ? "full" : "limited");

      dispatch({ type: "SET_VERIFIED" });
      toast({
        type: "success",
        message: data.isAdmin ? "Welcome back, Admin!" : "Signed in successfully",
        description: data.isAdmin ? "Full admin access granted" : "Limited access",
      });
      ...
      <DialogContent className="shadow-level-3 sm:max-w-md">
        <DialogHeader>
          <DialogTitle className="flex items-center gap-2">
            <HugeiconsIcon icon={ShieldCheck} className="size-5 text-foreground" />
