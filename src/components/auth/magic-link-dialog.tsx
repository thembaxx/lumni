...
function magicLinkReducer(state: MagicLinkForm, action: MagicLinkAction): MagicLinkForm {
  switch (action.type) {
    case "SET_EMAIL":
...
export function MagicLinkDialog({ open, onOpenChange, onSuccess: _onSuccess }: AuthDialogProps) {
  ...
  const handleSubmit = useCallback(async () => {
    ...
    if (!result.success) {
      const msg = result.error.issues[0]?.message || "Enter a valid email address";
    ...
    try {
      ...
      const msg = "We couldn&apos;t connect. Check your internet and try again.";
      dispatch({ type: "SET_ERROR", error: msg });
      toast({ type: "error", message: msg });
    } finally {
      setLoading(false);
  ...
  const handleResend = useCallback(async () => {
    ...
    try {
      ...
      if (!res.ok || !data.success) {
        ...
        const msg = data.error || "We couldn&apos;t resend that link. Try again in a moment?";
        dispatch({ type: "SET_ERROR", error: msg });
        toast({ type: "error", message: msg });
        return;
      }
      ...
      <DialogContent className="shadow-level-3 sm:max-w-md">
        <DialogHeader>
          <DialogTitle className="flex items-center gap-2">
            <HugeiconsIcon icon={Mail01Icon} className="size-5 text-foreground" />
        ...
        <div className="flex flex-col gap-4">
          {!sent ? (
            ...
            <>
              ...
              <ResendSection countdown={countdown} loading={loading} onResend={handleResend} />
            </>
          )}
        </div>
      </DialogContent>
