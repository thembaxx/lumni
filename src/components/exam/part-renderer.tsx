...
export function PartRenderer({ part, value, onChange, disabled }: PartRendererProps) {
  ...
  switch (part.type) {
    ...
    case "long-answer":
      return <LongAnswerInput value={partValue} onChange={onChange} disabled={disabled} />;

    case "essay":
      return <EssayInput value={partValue} onChange={onChange} disabled={disabled} />;

    case "calculation":
      return <CalculationInput value={partValue || ""} onChange={onChange} disabled={disabled} />;

    case "diagram":
      return <DiagramInput value={partValue} onChange={onChange} disabled={disabled} />;
    ...
    case "mixed":
      return (
        <MixedInput
          ...
          onChange={(subId, v) => {
            const current = {
              ...((typeof value === "object" && !Array.isArray(value) ? value : {}) as Record<
                string,
                string | string[]
              >),
            };
