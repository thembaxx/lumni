...
export function SchoolDetailsSection({
  ...
  const schoolTrailing = useMemo(
    () => (
      <EditableField value={schoolDraft} onSave={onSaveSchool} placeholder="Your school name" />
    ),
  ...
  const gradeTrailing = useMemo(
    () => <EditableField value={gradeDraft} onSave={onSaveGrade} placeholder="e.g. Grade 12" />,
    [gradeDraft, onSaveGrade],
  );
  const provinceLeading = useMemo(() => <HugeiconsIcon icon={MapPinIcon} className="size-5" />, []);
  ...
  return (
    <ListSection header="School Details (Optional)">
      <ListCell leading={schoolLeading} title="School" showSeparator trailing={schoolTrailing} />
      <ListCell leading={gradeLeading} title="Grade" showSeparator trailing={gradeTrailing} />
      <ListCell
