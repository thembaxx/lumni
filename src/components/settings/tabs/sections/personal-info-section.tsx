...
export function PersonalInfoSection({ user, isAnonymous, onUpdateName }: PersonalInfoSectionProps) {
  const displayNameLeading = useMemo(
    () => <HugeiconsIcon icon={UserIcon} className="size-5" />,
    [],
  );
  const displayNameTrailing = useMemo(
    () => <EditableField value={user?.name || ""} onSave={onUpdateName} placeholder="Your name" />,
    [user, onUpdateName],
  );
  const emailLeading = useMemo(() => <HugeiconsIcon icon={Mail01Icon} className="size-5" />, []);
  const emailTrailing = useMemo(
    () => <span className="max-w-40 truncate text-muted-foreground text-sm">{user?.email}</span>,
    [user],
  );
