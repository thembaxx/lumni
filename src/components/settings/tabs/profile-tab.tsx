...
const guidedSetupLeading = <HugeiconsIcon icon={CompassIcon} className="size-5" />;

const guidedSetupTrailing = (
  <span className="text-(length:--fs-footnote) font-semibold text-system-accent">Redo</span>
);
...
export function ProfileTab() {
  const { user, isAnonymous, updateProfile, verifyEmail, signOut, error } = useAuth();
  ...
  const { enrolledSubjects, subjects: allSubjects, isEnrolled } = useEnrolledSubjects();
  ...
  return (
    <div className="flex flex-col gap-10">
      ...
      {error && (
        <p className="ios-footnote -mt-6 text-center font-medium text-destructive">{error}</p>
      )}
      ...
      {user?.labels?.includes("student") && <ParentConsentSection userId={user.$id} />}
