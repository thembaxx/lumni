...
export function AiSolver({ cameraFocus, initialQuestion }: AiSolverProps) {
  return (
    <AppErrorBoundary>
      <AiSolverInner cameraFocus={cameraFocus} initialQuestion={initialQuestion} />
    </AppErrorBoundary>
  );
}
...
function AiSolverInner({ cameraFocus, initialQuestion }: AiSolverProps) {
  ...
  const [phase, setPhase] = useState<SolverPhase>(initialQuestion ? "confirm" : "input");
  ...
  return (
    <div className="flex h-full flex-col overflow-y-auto">
      ...
      {phase === "result" && result && (
        <SolverResultView subject={subject} result={result} onReset={handleReset} />
      )}
    </div>
  );
}
