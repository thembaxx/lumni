...
import { PlusIcon, Trash2Icon, CalculatorIcon } from "lucide-react";
...
const universityRequirements = [
  { university: "University of Cape Town (UCT)", minAPS: 33, courses: { medicine: 40, engineering: 36, commerce: 33, law: 35, science: 33 } },
  { university: "University of the Witwatersrand (Wits)", minAPS: 34, courses: { medicine: 38, engineering: 33, commerce: 32, law: 34, science: 30 } },
  { university: "University of Pretoria (UP)", minAPS: 28, courses: { medicine: 38, engineering: 35, commerce: 32, law: 32, science: 30 } },
  { university: "Stellenbosch University", minAPS: 28, courses: { medicine: 38, engineering: 35, commerce: 33, law: 33, science: 30 } },
  { university: "University of Johannesburg (UJ)", minAPS: 26, courses: { medicine: 35, engineering: 32, commerce: 30, law: 30, science: 28 } },
];
...
export function APSCalculator() {
  ...
  const addSubject = () => {
    setSubjects([...subjects, { id: Date.now().toString(), name: "", percentage: 0 }]);
  };
  ...
  const calculateAPS = (): number => {
    ...
    const filtered = scoredSubjects
      .map((score, idx) => ({ score, isLO: validSubjects[idx].name.toLowerCase().includes("life orientation") }))
