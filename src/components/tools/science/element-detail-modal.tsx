...
import { type Element, getGroup, getPeriod, getStateAtRoomTemp } from "@/lib/data/elements";
  ...
  return (
    <Dialog open={!!selectedElement} onOpenChange={(o) => !o && onClose()}>
      <DialogContent showCloseButton={false} className="max-w-md p-0 sm:max-w-md">
        <DialogTitle className="sr-only">{selectedElement?.name ?? "Element detail"}</DialogTitle>
        {selectedElement && (
          <m.div
          ...
          >
            ...
            <div className="p-6 pt-8">
              ...
              >
                ...
                >
                  <span className="font-extrabold text-3xl">{selectedElement.symbol}</span>
                </m.div>
                <div className="flex-1 pt-1">
                  <h2 className="mb-1 font-semibold text-2xl">{selectedElement.name}</h2>
              ...
              <div className="mb-4 grid grid-cols-2 gap-3 sm:grid-cols-4">
                ...
                >
                  ...
                  <p className="font-semibold text-sm">{getPeriod(selectedElement.row)}</p>
                </m.div>
                <m.div
                  initial={{ opacity: 0, y: 15 }}
                  animate={{ opacity: 1, y: 0 }}
                ...
                <m.div
                  ...
                  className="rounded-xl border border-white/5 bg-white/5 p-3 dark:border-white/10 dark:bg-white/10"
                >
                  <p className="mb-1 text-muted-foreground text-xs">State (25\u00B0C)</p>
                  <p className="flex items-center gap-1.5 font-semibold text-sm">
                    <span className="text-xs">
                      {STATE_ICONS[getStateAtRoomTemp(selectedElement.atomicNumber)]}
                    </span>
              ...
              >
                ...
                <p className="mb-3 font-semibold text-sm">{selectedElement.electronConfig}</p>
                <ElectronShellVisual electronConfig={selectedElement.electronConfig} />
              </m.div>
              ...
              >
                ...
                <p className="mb-1 font-semibold text-sm">{selectedElement.discoveryYear}</p>
