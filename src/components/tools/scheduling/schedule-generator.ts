...
export function generateDeterministicSchedule(
	...
	for (let day = 0; day < totalDays; day++) {
		...
		const sortedSubjects = subjects.toSorted((a, b) => {
			const aWeight =
				DIFFICULTY_WEIGHTS[a.difficulty] * (Math.random() * 0.3 + 0.7);
			const bWeight =
				DIFFICULTY_WEIGHTS[b.difficulty] * (Math.random() * 0.3 + 0.7);
			return bWeight - aWeight;
		});

		const topicsPerSubject: Record<string, string[]> = {};
		sortedSubjects.forEach((subj) => {
			topicsPerSubject[subj.id] = TOPIC_SUGGESTIONS[subj.id] || [
				`${subj.name} Study`,
			];
		});
