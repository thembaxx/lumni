...
type SnapPhase = "idle" | "capturing" | "extracting" | "confirm" | "solving" | "solved" | "error";
...
async function extractFromImage(dataUrl: string): Promise<{ text: string; fromCache: boolean }> {
  const hash = getImageHash(dataUrl);
  const cached = await _deps.db.extractionCache.where("imageHash").equals(hash).first();
...
export function SnapFab({ inline }: { inline?: boolean }) {
  ...
  const processCapture = useCallback(async (dataUrl: string) => {
    ...
    try {
      ...
      setError("Couldn't read the problem from the image. Try typing it instead.");
      setPhase("error");
    }
  }, []);
  ...
  return (
    <>
      ...
      <Dialog open={showCamera} onOpenChange={() => setShowCamera(false)}>
        <DialogContent showCloseButton={false} className="w-full max-w-md sm:max-w-md">
          <DialogTitle className="sr-only">Camera</DialogTitle>
          <CameraPreview onCapture={handleCameraCapture} onClose={() => setShowCamera(false)} />
        </DialogContent>
      </Dialog>
      ...
      {inline ? (
        <button
          ...
          className="flex size-11 shrink-0 items-center justify-center rounded-full bg-(--system-accent) text-white shadow-level-2 transition-transform hover:bg-(--system-accent)/90 active:scale-[0.96]"
        >
        ...
        <Button
          ...
          className={cn(
            ...
            "shadow-level-3 transition-transform active:scale-[0.96]",
