...
export function OrderingInput({ items, onSubmit }: OrderingInputProps) {
  ...
  const handleDrop = useCallback((e: React.DragEvent, targetId: string) => {
    ...
    setOrderedIds((prev) => {
      ...
      newOrder.splice(targetIdx >= 0 ? targetIdx : newOrder.length, 0, sourceId);
      return newOrder;
    });
  }, []);
              ...
              >
                ...
                <div className="flex shrink-0 gap-1">
                  <button
                    ...
                    onClick={() => moveUp(id)}
                    disabled={idx === 0}
                    className="flex min-h-9 min-w-9 items-center justify-center rounded-md text-muted-foreground text-xs hover:bg-muted focus-visible:ring-(--system-accent) focus-visible:ring-2 press-scale disabled:opacity-20"
                    aria-label="Move up"
                  >
                    ↑
                  </button>
