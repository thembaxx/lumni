...
export function DiagramLabellingInput({
	...
	const handleRegionDrop = useCallback(
		(e: React.DragEvent, regionId: string) => {
			...
			const otherRegion = Object.entries(placements).find(
				([, v]) => v === labelId,
			);
			setPlacements((prev) => {
	...
	const scaleX = useCallback(
		(regionW: number) => Math.round((regionW / width) * 100),
		[width],
	);
	const scaleY = useCallback(
		(regionH: number) => Math.round((regionH / height) * 100),
		[height],
	);
			...
			>
				...
				{regions.map((region) => {
					...
					return (
						...
						>
							{label ? (
								<span className="px-1 leading-tight">{label.text}</span>
							) : null}
						</button>
					);
				})}
