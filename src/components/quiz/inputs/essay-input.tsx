	...
	return (
		<div className="space-y-4">
				...
				<ul className="space-y-1">
					{rubric.map((c, i) => (
						<li key={i} className="text-sm text-muted-foreground">
							<span className="font-medium">{c.name}</span> ({c.maxScore} pts):{" "}
							{c.description}
						</li>
					))}
				</ul>
			...
			<div className="flex items-center justify-between">
				<span
					className={`text-xs ${words <= wordLimit ? "text-muted-foreground" : "text-destructive"}`}
				>
					{words}/{wordLimit} words
				</span>
				<Button
					onClick={() => onSubmit(value.trim())}
					disabled={disabled || words < 20 || words > wordLimit}
				>
					Submit Essay
				</Button>
			</div>
