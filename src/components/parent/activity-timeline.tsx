...
const activityIconVariants = cva("flex size-8 items-center justify-center rounded-lg", {
  variants: {
    variant: {
...
export function ActivityTimeline({ items, className, ...props }: ActivityTimelineProps) {
  if (items.length === 0) {
    return (
      ...
      >
        ...
        <p className="text-muted-foreground text-sm">No activity recorded this week.</p>
      </Card>
    );
  }
  ...
  return (
    <Card className={cn("p-4", className)} {...props}>
      <h3 className="mb-4 font-sans font-semibold text-sm tracking-tight">Recent Activity</h3>
...
function TimelineItem({ item, isLast }: { item: ActivityItem; isLast: boolean }) {
  return (
    <div className="relative flex gap-3 pb-4">
      {!isLast && (
        <div className="absolute top-8 left-4 h-full w-px bg-border" aria-hidden="true" />
      )}
      <div className={cn(activityIconVariants({ variant: item.type }), "z-elevated shrink-0")}>
