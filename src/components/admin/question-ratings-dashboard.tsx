...
export function QuestionRatingsDashboard() {
  ...
  return (
    <div className="flex flex-col gap-6">
      ...
      {stats.total > 0 && (
        <div className="rounded-2xl border border-border/80 bg-card p-4">
          <h3 className="mb-3 font-medium font-sans text-sm">Rating Distribution</h3>
          <div className="h-48">
            <BarChartComponent data={chartData} xKey="label" yKey="ratings" config={CHART_CONFIG} />
          </div>
        </div>
      )}
          ...
          <div className="flex max-h-60 flex-col gap-1 overflow-y-auto">
            {allRatings.map((r) => (
              ...
              >
                ...
                {r.feedback && (
                  <span className="max-w-48 truncate text-muted-foreground">"{r.feedback}"</span>
                )}
