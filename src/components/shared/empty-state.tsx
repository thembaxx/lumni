...
function EmptyTitle({ className, ...props }: React.ComponentProps<"div">) {
  return (
    <div
      ...
      className={cn("font-medium font-sans text-sm tracking-tight", className)}
      {...props}
    />
  );
}
  ...
  return (
    ...
    >
      <Empty>
        <EmptyHeader>
          {icon && (
            <HugeiconsIcon icon={icon} className="mx-auto size-10 text-muted-foreground/30" />
          )}
      ...
      <div className="relative mb-6">
        ...
        <div className="relative flex size-20 items-center justify-center rounded-full border border-muted-foreground/30 border-dashed bg-muted/30">
          {animation ? (
            ...
            <HugeiconsIcon icon={icon} className="size-8 text-muted-foreground/60" />
          ) : null}
        </div>
      </div>
      <h3 className="balance mb-2 w-full text-wrap text-center font-semibold text-xl">{title}</h3>
