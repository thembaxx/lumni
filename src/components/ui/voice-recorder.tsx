...
function deriveRecorderState(args: {
  ...
  audioBlob: Blob | null;
}): "idle" | "recording" | "recorded" | "playing" | "sending" | "permission-denied" {
...
}): "idle" | "recording" | "playing" | "permission-denied" | "validation-error" | "success" {
  if (args.showPermissionError) return "permission-denied";
...
export function VoiceRecorder({ onRecordingComplete, className }: VoiceRecorderProps) {
  ...
  const showPermissionError = permissionStatus === "denied" || permissionStatus === "unsupported";
  const showValidationError = !!(isTooShort || isTooLong || (recordingError && !isRecording));
  ...
  return (
    <div className={cn("mt-4 flex w-full flex-col items-center gap-6", className)}>
