...
const iconTransition = "transition-[opacity,filter,scale] duration-300 cubic-bezier(0.2, 0, 0, 1)";
...
export function PdfViewerImpl({ open, onOpenChange, exam }: PdfViewerProps) {
  ...
  const pdfUrl = cachedUrl || exam.fileUrl || exam.src || exam.localPath || exam.url;
  ...
  const handlePrevPage = useCallback(() => dispatchPdf({ type: "PREV_PAGE" }), []);
  const handleNextPage = useCallback(() => dispatchPdf({ type: "NEXT_PAGE" }), []);

  const handleZoomIn = useCallback(() => dispatchPdf({ type: "ZOOM_IN" }), []);
  const handleZoomOut = useCallback(() => dispatchPdf({ type: "ZOOM_OUT" }), []);
