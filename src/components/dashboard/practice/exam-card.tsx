...
export function ExamCard({ exam }: ExamCardProps) {
  ...
  const handleViewPdf = (e: React.MouseEvent) => {
    ...
    push(`/exam/${exam.id}/pdf`);
  };

  const handleSmartView = (e: React.MouseEvent) => {
    e.stopPropagation();
    setViewDropdownOpen(false);
    push(`/past-papers/${exam.id}`);
  };

  const handlePractice = (e: React.MouseEvent) => {
    e.stopPropagation();
    setPracticeDropdownOpen(false);
    push(`/quiz?subject=${encodeURIComponent(exam.subject ?? exam.title)}&count=10`);
  };
      ...
      <div className="flex min-w-0 flex-1 flex-col gap-1 pr-2">
        ...
        <div className="flex items-center gap-1.5">
          ...
          {exam.language && (
            <>
              <span className="text-muted-foreground/50 text-xs">·</span>
              <span className="text-muted-foreground text-xs capitalize">{exam.language}</span>
            </>
          )}
          {cached || exam.downloadedAt ? (
            <Badge variant="outline" className="ios-caption-3 h-5 px-1.5 text-muted-foreground/70">
              Saved
            </Badge>
          ) : null}
        </div>
          ...
          {cached || downloading ? (
            <span
              className={cn(
                ...
                downloading ? "animate-spin border-r-transparent" : "border-destructive",
              )}
            />
          ) : (
