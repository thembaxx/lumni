...
export function SubjectsDrawer({
  ...
  const { data: subjects, isLoading, error } = useFilteredSubjects(searchQuery, true);
  ...
  return (
    <Drawer direction="bottom">
      <DrawerTrigger asChild className="inline-flex items-center justify-center">
        {children}
      </DrawerTrigger>
      <DrawerContent className="mx-auto mt-0 min-h-[60dvh] max-w-lg animate-fade-in-scale rounded-b-2xl">
        ...
        <div className="max-h-[50dvh] grow overflow-y-auto px-4 pt-0 pb-4">
          {isLoading ? (
            <p className="py-8 text-center text-muted-foreground text-sm">Loading subjects…</p>
          ) : error ? (
            ...
            <p className="py-8 text-center text-muted-foreground text-sm">No subjects found.</p>
          ) : (
