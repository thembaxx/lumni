      ...
      <CardHeader className="flex flex-row items-center justify-between">
        ...
        <div className="flex items-center gap-2">
          ...
          <Input
            ...
            placeholder="10"
          />
          <Button size="sm" onClick={handleGenerate} disabled={generating || !selectedSubject}>
      ...
      <CardContent className="flex flex-col gap-3">
        ...
        {packs.length === 0 ? (
          <div className="flex items-center gap-3 rounded-xl bg-muted/30 p-2.5">
            <HugeiconsIcon icon={CloudOffIcon} className="size-4 text-muted-foreground" />
            <p className="text-muted-foreground text-sm">
              No offline packs yet. Select a subject and download questions for offline study.
            </p>
          </div>
        ) : (
              ...
              <div key={pack.id} className="flex items-center gap-3 rounded-xl bg-muted/30 p-2.5">
                ...
                <div className="min-w-0 flex-1">
                  ...
                  <p className="text-muted-foreground text-xs">
                    {pack.questionCount} questions · {formatBytes(pack.storageBytes)}
                    {pack.expiresAt && (
                      <>
                        {" · "}
                        <HugeiconsIcon
