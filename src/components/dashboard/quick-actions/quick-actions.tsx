		...
		<ul className="flex items-center gap-2 px-4 pb-3 overflow-x-auto scrollbar-hide animate-fade-in-up delay-400">
			{quickActions.map((action, index) => (
				<li key={action.label}>
					<Button
						variant="outline"
						className="rounded-full bg-secondary border-transparent text-foreground hover:bg-accent h-8 px-4 flex items-center gap-2 transition-transform duration-200 btn-ghost-hover"
