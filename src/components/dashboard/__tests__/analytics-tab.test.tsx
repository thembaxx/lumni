...
vi.mock("@/components/shared/stagger-provider", () => ({
  StaggerProvider: ({ children }: { children: React.ReactNode }) => <div>{children}</div>,
...
vi.mock("@/components/dashboard/analytics/comparative-analytics-panel", () => ({
  ComparativeAnalyticsPanel: () => <div data-testid="comparative-analytics-panel" />,
}));
...
vi.mock("next/dynamic", () => ({
  default: (
    ...
    const LazyComp = (_props: Record<string, unknown>) => {
      ...
      return options?.loading ? options.loading() : <div data-testid="dynamic-placeholder" />;
    };
    return LazyComp;
  },
}));
...
function createMockIntersectionObserver(triggerImmediately: boolean): typeof IntersectionObserver {
  return vi.fn(function (this: IntersectionObserver, callback: IntersectionObserverCallback) {
    const instance = {
