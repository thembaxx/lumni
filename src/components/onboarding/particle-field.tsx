...
const Particles = memo(function Particles({ step }: ParticlesProps) {
  ...
  const targetColor = useMemo(() => COLORS[step as keyof typeof COLORS] ?? COLORS[0], [step]);
  ...
  useFrame((state, delta) => {
    ...
    const positions2 = meshRef.current.geometry.attributes.position.array as Float32Array;
    for (let i = 0; i < PARTICLE_COUNT; i++) {
      const i3 = i * 3;
      const baseX = (i % 20) * 1.0 - 10;
      const baseY = Math.floor(i / 20) * 1.0 - 10;
      positions2[i3] = baseX + Math.sin(time * 0.3 + i * 0.1) * 0.8 + mouse.current.x * 0.2;
      positions2[i3 + 1] = baseY + Math.cos(time * 0.2 + i * 0.15) * 0.8 + mouse.current.y * 0.2;
