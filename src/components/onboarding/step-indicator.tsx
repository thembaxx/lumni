  ...
  return (
    <div className="flex flex-col gap-2">
      <div
        ...
        aria-label={labels ? labels[step] : `Step ${step + 1} of ${totalSteps}`}
      >
        {Array.from({ length: totalSteps }, (_, i) => (
          <m.div
            ...
            initial={false}
            animate={{
              backgroundColor: i <= step ? "var(--system-accent)" : "var(--system-separator)",
      ...
      {labels && <p className="text-center text-muted-foreground text-xs">{labels[step]}</p>}
    </div>
  );
}
