...
export function ConsentProvider({ children }: { children: React.ReactNode }) {
  ...
  useEffect(() => {
    if (!userId) {
      const cached = anonymousConsentCache.get("default") ?? readAnonymousConsent();
      if (cached) {
        ...
        setState({
          ...
          isLoading: false,
          needsTosAcceptance:
            !cached.tosVersion || cached.tosVersion !== appConfig.legal.tosVersion,
        });
    ...
    async function load() {
      ...
      const needsTosAcceptance = !existing?.tosVersion || existing.tosVersion !== currentTos;
  ...
  const updateConsent = useCallback(
    async (
      partial: Partial<
        Pick<
          UserConsent,
          "analytics" | "marketing" | "dataSharing" | "tosVersion" | "privacyVersion"
        >
      >,
    ) => {
      const now = nowISO();
      ...
      if (userId) {
        ...
        setState({
          ...
          isLoading: false,
          needsTosAcceptance:
            !updated.tosVersion || updated.tosVersion !== appConfig.legal.tosVersion,
        });
      } else {
        ...
        setState({
          consent: local,
          isLoading: false,
          needsTosAcceptance: !local.tosVersion || local.tosVersion !== appConfig.legal.tosVersion,
        });
      }
    },
    [userId],
