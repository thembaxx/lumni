...
describe("buildGenerateKey", () => {
  test("builds key with subject and topic slug", () => {
    expect(buildGenerateKey("mathematics", "Algebra")).toBe("gen:mathematics:algebra");
  });
  ...
  test("removes non-alphanumeric chars", () => {
    expect(buildGenerateKey("history", "World War II!!!")).toBe("gen:history:world-war-ii");
  });
  ...
  test("collapses repeated non-alphanumeric chars", () => {
    expect(buildGenerateKey("biology", "cell---structure")).toBe("gen:biology:cell-structure");
  });
});
