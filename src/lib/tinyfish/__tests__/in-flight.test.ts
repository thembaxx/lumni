...
import { clearInFlight, deduped, hasInFlight, inFlightSize } from "../in-flight";
...
describe("deduped", () => {
  test("runs factory once when called multiple times with same key", async () => {
    ...
    const promises = [deduped("key1", factory), deduped("key1", factory), deduped("key1", factory)];
...
describe("inFlightSize", () => {
  test("tracks in-flight count", async () => {
    const p1 = deduped("a", () => new Promise((r) => setTimeout(() => r(1), 10)));
    const p2 = deduped("b", () => new Promise((r) => setTimeout(() => r(2), 10)));
