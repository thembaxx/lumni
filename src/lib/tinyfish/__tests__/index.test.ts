...
const usageStore = new Map<string, { id: number; userId: string; date: string; count: number }>();
...
vi.mock("@/lib/db", () => ({
  dexieDataAccess: {
    tinyfishCache: {
      ...
      put(value: { key: string; value: unknown; expiresAt: number; fetchedAt: number }) {
            ...
            return {
              async first() {
                for (const entry of usageStore.values()) {
                  if (entry.userId === userId && entry.date === date) return entry;
                }
                return undefined;
              },
            };
