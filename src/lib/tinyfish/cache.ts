...
export async function setCached(key: string, value: RagContext, ttlMs: number): Promise<void> {
  const now = Date.now();
...
export async function clearExpiredCache(): Promise<number> {
  const now = Date.now();
  const expired = (await db().tinyfishCache.where("expiresAt").below(now).toArray()).map(
    (e) => e.key,
  );
  if (expired.length > 0) {
...
export async function getTodayUsageCount(userId: string): Promise<number> {
  ...
  const entry = await db().tinyfishUsage.where("[userId+date]").equals([userId, date]).first();
  return entry?.count ?? 0;
}

export async function incrementTodayUsage(userId: string): Promise<number> {
  const date = todayDateKey();
  const existing = await db().tinyfishUsage.where("[userId+date]").equals([userId, date]).first();
