...
export class ExamDownloadService {
  private getExamsFromJson(year: number, session: string, subjectId: string): ExamPaperEntry[] {
    try {
      const filePath = path.resolve("data", "exams", "index.json");
      const content = fs.readFileSync(filePath, "utf-8");
      const data: ExamsData = JSON.parse(content);
      return data.exams.filter(
        (e) => e.year === year && e.session === session && e.subjectId === subjectId,
      );
    } catch {
      return [];
    }
  ...
  private findExamPaperUrl(
    ...
    const exams = this.getExamsFromJson(year, session, this.normalizeSubjectToCode(subjectName));
    const exam = exams.find((e) => e.paperNumber === paperNumber && e.type === type);
    if (exam?.url) return { url: exam.url, title: exam.title };
    return null;
  }

  private async downloadPdf(url: string): Promise<{ buffer: Buffer; contentType: string } | null> {
    try {
  ...
  private async processExamFlow(params: {
    ...
    const { subjectName, subjectId, year, paperNum, examType, type, session, fileName } = params;
    const label = type === "paper" ? `P${paperNum}` : `Memo P${paperNum}`;

    const examInfo = this.findExamPaperUrl(subjectName, year, paperNum, type, session);
    if (!examInfo) {
      return {
        downloaded: 0,
    ...
    const uploadResult = await uploadToUploadThing(new Uint8Array(pdfData.buffer), fileName);
    if (!uploadResult) {
      return {
        downloaded: 0,
  ...
  private async downloadSubjectPapers(params: {
    ...
    const { subject: subj, examType, session, year, paperNumbers, includeMemo } = params;
    ...
    const paperResults = await Promise.all(
      paperNumbers.map(async (paperNum) => {
        ...
        const paperDownloaded = flowResults.reduce((sum, r) => sum + r.downloaded, 0);
        const localErrors = flowResults.flatMap((r) => r.errors);
        return { paperDownloaded, localErrors };
      }),
    );

    const downloadedForSubj = paperResults.reduce((sum, r) => sum + r.paperDownloaded, 0);
  ...
  async download(request: DownloadRequest): Promise<DownloadResponse> {
    ...
    const subjectQueries = subjectIds.flatMap((id) => [Query.equal("code", id)]);
    const subjects = await listDocuments<Subject>(COLLECTIONS.SUBJECTS, [
      ...(subjectQueries.length > 1 ? [Query.or(subjectQueries)] : subjectQueries),
    ]);
