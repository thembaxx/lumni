...
describe("getReadableErrorMessage", () => {
  ...
  test("translates 'already exists' to sign-in prompt", () => {
    const result = getReadableErrorMessage(new Error("A user with this email already exists"));
    expect(result).toBe("An account with this email already exists. Sign in instead.");
  });

  test("translates 'already registered' to sign-in prompt", () => {
    const result = getReadableErrorMessage(new Error("Account already registered"));
    expect(result).toBe("An account with this email already exists. Sign in instead.");
  });
  ...
  test("translates 'user not found'", () => {
    const result = getReadableErrorMessage(new Error("User with this email not found"));
    expect(result).toBe("No account found with this email. Create an account instead.");
  });
  ...
  test("translates network errors", () => {
    ...
    expect(result).toBe("Couldn't connect. Check your internet connection and try again.");
  });
  ...
  test("translates expired verification link", () => {
    const result = getReadableErrorMessage(new Error("Verification token is invalid"));
    expect(result).toBe("This verification link has expired or is invalid. Request a new one.");
  });
});
