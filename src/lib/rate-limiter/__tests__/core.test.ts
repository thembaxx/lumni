import { describe, expect, test } from "bun:test";
import { getRateLimitHeaders, RateLimiter } from "../core";

describe("RateLimiter", () => {
	const config = { max: 3, windowMs: 60000 };

	test("allows first request", async () => {
		const limiter = new RateLimiter();
		const result = await limiter.check("key1", config);
		expect(result.allowed).toBe(true);
		expect(result.remaining).toBe(2);
	});

	test("decrements remaining on each request", async () => {
		const limiter = new RateLimiter();
		await limiter.check("key2", config);
		const r2 = await limiter.check("key2", config);
		expect(r2.remaining).toBe(1);
		const r3 = await limiter.check("key2", config);
		expect(r3.allowed).toBe(true);
		expect(r3.remaining).toBe(0);
	});

	test("blocks when limit exceeded", async () => {
		const limiter = new RateLimiter();
		await limiter.check("key3", config);
		await limiter.check("key3", config);
		await limiter.check("key3", config);
		const r4 = await limiter.check("key3", config);
		expect(r4.allowed).toBe(false);
		expect(r4.remaining).toBe(0);
	});

	test("different keys have independent counters", async () => {
		const limiter = new RateLimiter();
		await limiter.check("a", config);
		await limiter.check("a", config);
		const rb = await limiter.check("b", config);
		expect(rb.allowed).toBe(true);
		expect(rb.remaining).toBe(2);
	});

	test("reset clears a key", async () => {
		const limiter = new RateLimiter();
		await limiter.check("key4", config);
		await limiter.check("key4", config);
		await limiter.check("key4", config);
		await limiter.reset("key4");
		const r = await limiter.check("key4", config);
		expect(r.allowed).toBe(true);
		expect(r.remaining).toBe(2);
	});

	test("cleanup removes expired entries", async () => {
		const limiter = new RateLimiter();
		await limiter.check("expire", { max: 1, windowMs: -1 });
		expect(await limiter.getStoreSize()).toBe(1);
		await limiter.cleanup();
		expect(await limiter.getStoreSize()).toBe(0);
	});

	test("supports different configs per key", async () => {
		const limiter = new RateLimiter();
		const strict = { max: 1, windowMs: 60000 };
		await limiter.check("strict", strict);
		const r2 = await limiter.check("strict", strict);
		expect(r2.allowed).toBe(false);
	});

	test("resetAt is in the future", async () => {
		const limiter = new RateLimiter();
		const result = await limiter.check("future", config);
		expect(result.resetAt).toBeGreaterThan(Date.now() - 1000);
	});
});

describe("getRateLimitHeaders", () => {
	test("returns correct headers for allowed request", () => {
		const headers = getRateLimitHeaders({
			allowed: true,
			remaining: 4,
			resetAt: 1000,
		});
		expect(headers["X-RateLimit-Remaining"]).toBe("4");
		expect(headers["X-RateLimit-Reset"]).toBe("1000");
	});

	test("returns Retry-After for blocked request", () => {
		const future = Date.now() + 30000;
		const headers = getRateLimitHeaders({
			allowed: false,
			remaining: 0,
			resetAt: future,
		});
		expect(headers["Retry-After"]).toBe("30");
	});
});
