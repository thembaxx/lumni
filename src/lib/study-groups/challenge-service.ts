...
function computeCombinedScore(
  ...
  const questionScore = (questionsAnswered / Math.max(1, groupTotalQuestions)) * 30;
...
export async function getOrCreateChallenge(
  ...
  try {
    ...
    const existing = await listDocuments<GroupChallenge>(COLLECTIONS.GROUP_CHALLENGES, [
      `groupId=${groupId}`,
      `status=active`,
    ]);

    const activeChallenge = existing.find(
      (c) => c.weekStart <= end && c.weekEnd >= start && c.challengeType === challengeType,
    );
    ...
    const challenge = await getDocument<GroupChallenge>(COLLECTIONS.GROUP_CHALLENGES, challengeId);
    if (!challenge) return failure("Failed to create challenge");
    return success(challenge);
  } catch (err) {
    logError("StudyGroupsChallengeService", err);
    return failure(err instanceof Error ? err.message : "Failed to get challenge");
  }
}
...
export async function createCustomChallenge(
  ...
  try {
    ...
    logError("StudyGroupsChallengeService", err);
    return failure(err instanceof Error ? err.message : "Failed to create challenge");
  }
}
...
export async function getChallengeEntries(
  ...
  try {
    const entries = await listDocuments<GroupChallengeEntry>(COLLECTIONS.GROUP_CHALLENGE_ENTRIES, [
      `challengeId=${challengeId}`,
    ]);
    return success(entries);
  } catch (err) {
    logError("StudyGroupsChallengeService", err);
    return failure(err instanceof Error ? err.message : "Failed to get entries");
  }
}
...
export async function updateChallengeEntry(
  ...
  try {
    const memberships = await listDocuments<Record<string, unknown>>(COLLECTIONS.GROUP_MEMBERS, [
      `userId=${userId}`,
    ]);
    ...
    await Promise.all(
      memberships.map(async (membership) => {
        ...
        const [existing, allEntries] = await Promise.all([
          listDocuments<GroupChallengeEntry>(COLLECTIONS.GROUP_CHALLENGE_ENTRIES, [
        ...
        const groupTotalXp = allEntries.reduce((s, e) => s + e.xpEarned, 0) + xpGained;
        const groupTotalQ =
          allEntries.reduce((s, e) => s + e.questionsAnswered, 0) + questionsCount;

        if (existing.length > 0) {
          const entry = existing[0];
          const newXp = entry.xpEarned + xpGained;
          const newQ = entry.questionsAnswered + questionsCount;
          const combined = computeCombinedScore(newXp, newQ, accuracy, groupTotalXp, groupTotalQ);
...
async function closeChallenge(challengeId: string): Promise<void> {
  try {
    ...
    const ranked = entries.toSorted((a, b) => b.combinedScore - a.combinedScore);
...
export async function getGroupBadges(groupId: string): Promise<ServiceResult<GroupBadge[]>> {
  try {
...
> {
  try {
    ...
    const challenges = await listDocuments<GroupChallenge>(COLLECTIONS.GROUP_CHALLENGES, [
      `weekStart=${start}`,
      `status=active`,
    ]);
    ...
    logError("StudyGroupsChallengeService", err);
    return failure(err instanceof Error ? err.message : "Failed to get leaderboard");
  }
}
