...
async function dequeueOnce<T extends QueueItemBase>(queue: QueueCore<T>): Promise<T | null> {
...
export class QueueCore<T extends QueueItemBase> {
  ...
  async next(): Promise<T | null> {
    ...
    available.sort((a, b) => b.priority - a.priority || a.createdAt - b.createdAt);
    return available[0];
  }
  ...
  async processBatch(
    ...
    if (concurrencyGuard.isProcessing) return { processed: 0, succeeded: 0, failed: 0 };
  ...
  async resetStaleProcessingItems(staleThreshold = 60000): Promise<number> {
    ...
    const stale = all.filter(
      (j) => j.status === "processing" && j.startedAt && now - j.startedAt > staleThreshold,
    );
    await Promise.all(
      stale.flatMap((item) =>
        item.id
  ...
  async getStats(): Promise<{
    ...
    const all = await this.table.toArray();
    const count = (status: string) => all.filter((j) => j.status === status).length;
    return {
