...
export async function trackQuestionResult(params: TrackResultParams): Promise<void> {
  const { subjectId, topicId, bloomLevel, score, maxScore, questionType, correct } = params;
  ...
  await Promise.allSettled([
    ...
    params.userId ? updateChallengeEntry(params.userId, xpGained, 1, accuracy) : Promise.resolve(),
  ]);
}

export function isPassingScore(score: number, maxScore: number, threshold = 0.5): boolean {
