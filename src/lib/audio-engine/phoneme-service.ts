...
function backtraceAlignment(
  ...
  while (i > 0 || j > 0) {
    if (i > 0 && j > 0 && matrix[i][j] === matrix[i - 1][j - 1] + (a[j - 1] === b[i - 1] ? 0 : 1)) {
      result.unshift({
        expected: a[j - 1],
        actual: b[i - 1],
        correct: a[j - 1] === b[i - 1],
...
export function alignPhonemes(studentText: string, expectedText: string): PhonemeAlignment[] {
  ...
  const maxWords = Math.max(expectedPhonemeWords.length, studentPhonemeWords.length);
  ...
  for (let w = 0; w < maxWords; w++) {
    ...
    const wordAlignments = backtraceAlignment(expectedPhonemes, studentPhonemes, matrix);
...
export function assessPhonemes(studentText: string, expectedText: string): PhonemeResult {
  const details = alignPhonemes(studentText, expectedText);

  const correctCount = details.filter((d) => d.correct).length;
  const totalCount = details.length;
  const phonemeAccuracy = totalCount > 0 ? Math.round((correctCount / totalCount) * 100) : 100;
