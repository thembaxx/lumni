...
beforeEach(() => {
  cache.clear();
  __setDepsForTesting({
    db: { dictionaryCache: mockDictionaryCache } as Parameters<typeof __setDepsForTesting>[0]["db"],
  });
...
describe("lookupWord", () => {
  ...
  test("fetches from API on cache miss", async () => {
    ...
    expect(fetchMock).toHaveBeenCalledWith("https://api.dictionaryapi.dev/api/v2/entries/en/hello");
  });
  ...
  test("caches result after API fetch", async () => {
    ...
    const putArg = mockDictionaryCache.put.mock.calls[0]?.[0] as DictionaryCacheEntry | undefined;
