...
import { allocateDailyMinutes, calculateSubjectWeights, generateStudyPlan } from "../algorithms";
...
describe("generateStudyPlan", () => {
  ...
  test("schedules topics across valid weekdays only", async () => {
    ...
    const plan = await generateStudyPlan(baseSettings, subjects);
    const scheduledDates = plan.topics.flatMap((t) => (t.scheduledDate ? [t.scheduledDate] : []));
  ...
  test("distributes topics across subjects based on weights", async () => {
    ...
    expect(mathTopics[0].estimatedMinutes).toBeGreaterThan(englishTopics[0].estimatedMinutes);
  });
