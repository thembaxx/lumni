...
interface AIHandlerConfig<T> {
	...
	errorLabel: string;
	service: {
		execute: (body: T, helpers: { userId: string; requestId: string }) => Promise<Record<string, unknown>>;
	};
...
export function createAIHandler<T>(
	config: AIHandlerConfig<T>,
) {
	const { budgetType, errorLabel, service, parseBody, validate, useRateLimit = true } = config;
	...
	const handler = async (req: NextRequest) => {
		...
		try {
			...
			if (!budget.allowed) {
				return budget.response ?? NextResponse.json({ error: "Budget exceeded" }, { status: 429 });
			}
