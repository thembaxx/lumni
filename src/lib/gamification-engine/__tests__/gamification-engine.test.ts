...
describe("GamificationEngine", () => {
  ...
  describe("addXp", () => {
    ...
    test("awards bonus XP when challenge becomes newly completed", () => {
      ...
      const questionsChallenge = result.dailyChallenges.find((c) => c.id === "daily_questions");
  ...
  describe("addAchievement", () => {
    test("adds a new achievement with XP reward", () => {
      ...
      const { data: result, achievement } = engine.addAchievement(data, achievementId);
    ...
    test("does not duplicate existing achievements", () => {
      ...
      const data = {
        ...makeDefault(),
        achievements: [{ id: achievementId, earnedAt: "2026-01-01T00:00:00.000Z" }],
      };
