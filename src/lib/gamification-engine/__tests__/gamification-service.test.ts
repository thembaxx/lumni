...
const {
	...
	return {
		loadMock: vi.fn(() => ({ ...def })),
		mergeWithDefaultsMock: vi.fn(
			(d: Record<string, unknown>) => d as typeof def,
		),
...
describe("GamificationService", () => {
	...
	describe("subscribe / notify", () => {
		test("listener receives state on mutation", () => {
			...
			expect(listener).toHaveBeenCalledWith(
				expect.objectContaining({ xp: 10, totalXp: 10 }),
			);
		});
		...
		test("delegates to engine and returns AchievementResult", () => {
			...
			addAchievementMock.mockReturnValue({
				data: {
					...defaultState,
					achievements: [
						{ id: "first_question", earnedAt: "2026-01-01T00:00:00.000Z" },
					],
					totalXp: 50,
				},
				achievement: {
		...
		test("persists and notifies after mutation", () => {
			...
			addAchievementMock.mockReturnValue({
				data: {
					...defaultState,
					achievements: [
						{ id: "streak_3", earnedAt: "2026-06-01T00:00:00.000Z" },
					],
					totalXp: 100,
				},
	...
	describe("checkForRewardChests", () => {
		test("with claimable chest: persists and notifies", () => {
			...
			const chestData = {
				...defaultState,
				claimedChests: [
					{ id: "chest_1000", claimedAt: "2026-06-21T00:00:00.000Z" },
				],
				totalXp: 1100,
			};
		...
		test("without claimable chest: still notifies, no persist", () => {
			...
			checkAndClaimRewardChestMock.mockImplementation(
				(data: Record<string, unknown>) => ({ data, chest: null }),
			);
	...
	describe("error handling", () => {
		...
		test("API sync failure: swallowed, logged", async () => {
			...
			expect(logErrorMock).toHaveBeenCalledWith(
				"GamificationService.syncToServer",
				syncError,
			);
