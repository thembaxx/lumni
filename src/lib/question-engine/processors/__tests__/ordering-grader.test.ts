...
function makeQuestion(overrides?: Partial<Question<"ordering">>): Question<"ordering"> {
  return {
    id: "q1",
    type: "ordering",
    subject: "mathematics",
...
describe("Ordering Grader", () => {
  ...
  test("incomplete order scores zero", () => {
    const q = makeQuestion();
    const result = grade(q, { type: "ordered-items", value: ["a", "b"] }, {} as PromptManager);
    expect(result.correct).toBe(false);
    expect(result.score).toBe(0);
  });
  ...
  test("empty answer scores zero", () => {
    const q = makeQuestion();
    const result = grade(q, { type: "ordered-items", value: [] }, {} as PromptManager);
    expect(result.correct).toBe(false);
    expect(result.score).toBe(0);
  });
});
