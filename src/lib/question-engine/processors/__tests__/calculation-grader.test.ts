...
describe("Calculation Grader", () => {
	test("exact answer with unit is correct", () => {
		...
		const result = grade(q, { type: "numeric", value: { value: 10, unit: "N" } }, {} as PromptManager);
	...
	test("answer within tolerance is correct", () => {
		const q = makeQuestion();
		const result = grade(q, { type: "numeric", value: { value: 10.05, unit: "N" } }, {} as PromptManager);
		expect(result.correct).toBe(true);
	});

	test("answer outside tolerance is incorrect", () => {
		const q = makeQuestion();
		const result = grade(q, { type: "numeric", value: { value: 11, unit: "N" } }, {} as PromptManager);
	...
	test("correct value but wrong unit gets partial score", () => {
		const q = makeQuestion();
		const result = grade(q, { type: "numeric", value: { value: 10, unit: "m/s" } }, {} as PromptManager);
		expect(result.correct).toBe(false);
	...
	test("no answer provided is incorrect", () => {
		const q = makeQuestion();
		const result = grade(q, { type: "numeric", value: {} }, {} as PromptManager);
		expect(result.correct).toBe(false);
		expect(result.score).toBe(0);
	});
