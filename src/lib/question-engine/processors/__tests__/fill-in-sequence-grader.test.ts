...
describe("FillInSequence Grader", () => {
  test("all correct answers get full points", () => {
    ...
    const result = grade(q, { type: "sequence-blanks", value: { b1: "O₂" } }, {} as PromptManager);
  ...
  test("wrong answer scores zero", () => {
    const q = makeQuestion();
    const result = grade(q, { type: "sequence-blanks", value: { b1: "H₂O" } }, {} as PromptManager);
  ...
  test("empty answer scores zero", () => {
    const q = makeQuestion();
    const result = grade(q, { type: "sequence-blanks", value: {} }, {} as PromptManager);
    expect(result.correct).toBe(false);
    expect(result.score).toBe(0);
  });
});
