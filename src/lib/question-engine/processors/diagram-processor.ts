...
export class DiagramProcessor implements QuestionProcessor<"diagram"> {
	...
	async generate(params: GenerationParams): Promise<Question<"diagram">[]> {
		...
		const parsed = JSON.parse(this.cleanResponse((result as AIResponse).content)) as Question<"diagram">[];
	...
	async grade(question: Question<"diagram">, answer: UserAnswer): Promise<GradingResult> {
		...
		} catch { return { correct: false, score: 0, maxScore: question.points, feedback: question.explanation }; }
	}

	validate(question: Question<"diagram">): ValidationResult {
		const errors = [];
		if (!question.body.diagramData) errors.push({ type: "schema" as const, field: "diagramData", message: "Diagram data required", severity: "error" as const });
