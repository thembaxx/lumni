...
export class EssayProcessor implements QuestionProcessor<"essay"> {
	...
	async generate(params: GenerationParams): Promise<Question<"essay">[]> {
		...
		const parsed = JSON.parse(this.cleanResponse((result as AIResponse).content)) as Question<"essay">[];
	...
	async grade(question: Question<"essay">, answer: UserAnswer): Promise<GradingResult> {
		const studentAnswer = answer.value as string;
		if (!studentAnswer || studentAnswer.trim().length < 20) {
			return { correct: false, score: 0, maxScore: question.points, feedback: "Essay is too short to grade." };
		}
		...
		if ("available" in result && !result.available) return { correct: false, score: 0, maxScore: question.points, feedback: "Grading unavailable currently." };
		try {
		...
		} catch { return { correct: false, score: 0, maxScore: question.points, feedback: "Unable to grade essay." }; }
	}

	validate(question: Question<"essay">): ValidationResult {
		const errors = [];
		if (!question.body.rubric || question.body.rubric.length < 2) errors.push({ type: "schema" as const, field: "rubric", message: "Need at least 2 rubric criteria", severity: "error" as const });
		if (!question.body.modelAnswer || question.body.modelAnswer.length < 20) errors.push({ type: "schema" as const, field: "modelAnswer", message: "Model answer too short", severity: "error" as const });
