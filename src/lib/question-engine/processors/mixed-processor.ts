...
export class MixedProcessor implements QuestionProcessor<"mixed"> {
	...
	async generate(params: GenerationParams): Promise<Question<"mixed">[]> {
		...
		const parsed = JSON.parse(this.cleanResponse((result as AIResponse).content)) as Question<"mixed">[];
	...
	async grade(question: Question<"mixed">, answer: UserAnswer): Promise<GradingResult> {
		const parts = answer.value as { partId: string; answer: UserAnswer }[];
		if (!parts?.length) return { correct: false, score: 0, maxScore: question.points, feedback: "No answers." };
		...
		} catch { return { correct: false, score: 0, maxScore: question.points, feedback: "Unable to grade mixed answer." }; }
	}

	validate(question: Question<"mixed">): ValidationResult {
		const errors = [];
		if (!question.body.parts?.length) errors.push({ type: "schema" as const, field: "parts", message: "Parts required", severity: "error" as const });
