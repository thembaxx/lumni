...
export function formatCorrectAnswer(question: Question): string {
  ...
  switch (question.type) {
    case "ordering": {
      ...
      return correctOrder.map((id, i) => `${i + 1}. ${itemMap.get(id) ?? id}`).join("\n");
    }

    case "fill-in-sequence": {
      const blanks = body.blanks as { id: string; correctAnswer: string }[] | undefined;
      if (!blanks) return question.explanation;
      return blanks.map((b, i) => `Blank ${i + 1}: ${b.correctAnswer}`).join("\n");
    }

    case "match-pairs": {
      const leftItems = body.leftItems as { id: string; text: string }[] | undefined;
      const correctMatches = body.correctMatches as
        | { leftId: string; rightId: string }[]
        | undefined;
      const rightItems = body.rightItems as { id: string; text: string }[] | undefined;
    ...
    case "diagram-labelling": {
      const regions = body.regions as { id: string; label: string }[] | undefined;
    ...
    case "hot-spot": {
      const hsRegions = body.regions as { id: string; label: string }[] | undefined;
      ...
      return correct ? `Correct region: ${correct.label}` : question.explanation;
    }
