...
export async function loadQuestionsFromAppwrite(
  ...
  try {
    ...
    const response = await databases.listDocuments(APPWRITE_DATABASE_ID, COLLECTION_ID, queries);
    return response.documents.reduce((acc: Question[], doc: Record<string, unknown>) => {
