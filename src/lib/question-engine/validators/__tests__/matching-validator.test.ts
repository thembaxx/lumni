...
function makeQuestion(overrides?: Partial<Question<"matching">>): Question<"matching"> {
	return {
		...
		body: { pairs: [{ left: "France", right: "Paris" }, { left: "Germany", right: "Berlin" }], shuffle: true },
		...overrides,
	};
}
...
describe("Matching Validator", () => {
	...
	test("fails on less than 2 pairs", () => {
		const result = validateQuestion(makeQuestion({ body: { pairs: [{ left: "A", right: "B" }], shuffle: true } }));
		expect(result.isValid).toBe(false);
	});

	test("fails on duplicate left items", () => {
		const result = validateQuestion(makeQuestion({ body: { pairs: [{ left: "A", right: "B" }, { left: "A", right: "C" }, { left: "D", right: "E" }], shuffle: true } }));
		expect(result.isValid).toBe(false);
	});

	test("fails on duplicate right items", () => {
		const result = validateQuestion(makeQuestion({ body: { pairs: [{ left: "A", right: "X" }, { left: "B", right: "X" }, { left: "C", right: "Y" }], shuffle: true } }));
		expect(result.isValid).toBe(false);
	});
});
