...
function makeQuestion(overrides?: Partial<Question<"source-based">>): Question<"source-based"> {
	return {
		...
		body: {
			...
			subQuestions: [
				{ id: "sq1", questionText: "When?", type: "short-answer", points: 4, body: { modelAnswer: "1961", acceptableAnswers: ["1961"], maxLength: 50 } },
			],
		},
		...overrides,
	};
...
describe("Source-based Validator", () => {
	...
	test("fails on missing source", () => {
		const result = validateQuestion(makeQuestion({ body: { source: undefined!, subQuestions: [] } }));
		expect(result.isValid).toBe(false);
	});

	test("fails on empty subQuestions", () => {
		const result = validateQuestion(makeQuestion({ body: { source: { type: "text", content: "A substantial source text that is long enough to pass validation checks for the source content requirement." }, subQuestions: [] } }));
		expect(result.isValid).toBe(false);
	});
});
