...
export function validateEssay(question: Question<"essay">): ValidationResult {
	...
	if (!question.body.rubric || question.body.rubric.length < 2) {
		errors.push({ type: "schema", field: "rubric", message: "Need at least 2 rubric criteria", severity: "error" });
	}

	if (!question.body.modelAnswer || question.body.modelAnswer.trim().length < 50) {
		warnings.push({ type: "quality", field: "modelAnswer", message: "Essay model answer should be substantial", severity: "warning" });
	}

	if (question.body.wordLimit < 100) {
		warnings.push({ type: "quality", field: "wordLimit", message: "Word limit seems very low for an essay", severity: "warning" });
	}
