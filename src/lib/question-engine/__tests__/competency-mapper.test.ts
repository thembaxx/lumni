...
describe("mapCompetencyToBloomList", () => {
  test("returns default list for undefined level", () => {
    expect(mapCompetencyToBloomList(undefined)).toEqual(["remember", "understand", "apply"]);
  });

  test("returns fallback for invalid level", () => {
    expect(mapCompetencyToBloomList("invalid" as never)).toEqual(["remember", "understand"]);
  });

  test("novice returns remember and understand", () => {
    expect(mapCompetencyToBloomList("novice")).toEqual(["remember", "understand"]);
  });

  test("developing returns understand and apply", () => {
    expect(mapCompetencyToBloomList("developing")).toEqual(["understand", "apply"]);
  });

  test("proficient returns apply, analyze, evaluate", () => {
    expect(mapCompetencyToBloomList("proficient")).toEqual(["apply", "analyze", "evaluate"]);
  });

  test("mastered returns evaluate and create", () => {
    expect(mapCompetencyToBloomList("mastered")).toEqual(["evaluate", "create"]);
  });
});
