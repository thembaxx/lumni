...
describe("PromptManager", () => {
  ...
  test("getPrompt returns correct structure for multiple-choice type", () => {
    ...
    expect(result.user).toContain("Generate 3 multiple-choice questions for physics");
  ...
  test("getHintPrompt returns correct structure", () => {
    ...
    expect(result.user).toContain("Generate a hint for this calculation question");
  });
  ...
  test("buildCompetencyContext returns formatted string when competency data provided", () => {
    ...
    expect(result).toContain("Student context: The student has a proficient understanding");
  ...
  test("getPrompt injects XML into user prompt when ragContext has sources", () => {
    ...
    const result = promptManager.getPrompt("multiple-choice", params, ragContext);
