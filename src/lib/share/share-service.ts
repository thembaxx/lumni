...
export async function shareQuestion(
  ...
  try {
    await _deps.db.sharedQuestions.add(record as unknown as SharedQuestionRecord);
  } catch (err) {
  ...
  syncManager.enqueue({
    ...
    payload: {
      ...
      sharedById: userId,
      sharedAt: Date.now(),
      sources: sources && sources.length > 0 ? JSON.stringify(sources) : undefined,
    },
  });
...
export async function getSharedQuestion(id: string): Promise<SharedRecord | null> {
  try {
