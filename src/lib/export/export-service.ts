  ...
  async buildFullReport(): Promise<FullReport> {
    ...
    return {
              ...
              if (a.earnedAt)
                acc.push({
                  ...
                  ...((a as unknown as Record<string, unknown>).name
                    ? {
                        name: (a as unknown as Record<string, unknown>).name as string,
                      }
                    : {}),
                  ...((a as unknown as Record<string, unknown>).rarity
                    ? {
                        rarity: (a as unknown as Record<string, unknown>).rarity as string,
                      }
                    : {}),
                });
              return acc;
      ...
      quizHistory: quizAttempts.map((a: QuizAttempt) => ({
        ...
        accuracy: a.totalQuestions > 0 ? Math.round((a.score / a.totalQuestions) * 100) : 0,
        duration: a.duration,
      ...
      competency: competencies.reduce<Record<string, { topics: number; averageScore: number }>>(
        (acc: Record<string, { topics: number; averageScore: number }>, c: CompetencyRecord) => {
          ...
          acc[c.subjectId].averageScore =
            (acc[c.subjectId].averageScore * (acc[c.subjectId].topics - 1) + c.score) /
            acc[c.subjectId].topics;
          return acc;
        },
        {},
  ...
  toCSV(attempts: QuizAttempt[], examSessions: ExamSessionSnapshot[]): string {
    ...
    if (attempts.length > 0) {
      ...
      for (const a of attempts) {
        const accuracy = a.totalQuestions > 0 ? Math.round((a.score / a.totalQuestions) * 100) : 0;
        lines.push(
          [
            "Quiz",
