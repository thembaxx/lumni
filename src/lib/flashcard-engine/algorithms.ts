...
const W: number[] = [
  0.4197, 1.1869, 3.0412, 15.2441, 7.1434, 0.6477, 1.0007, 0.0675, 0.0025, 1.0138, 0.1868, 0.3813,
  1.4039, 0.9125, 0.0561, 0.2119, 2.0954, 0.6835, 2.2315,
];
...
export function calculateNextReviewFSRS(
  ...
  if (g < 3) {
    newStability = W[6] * D ** -W[10] * ((S + 1) ** W[11] - 1) * Math.exp((1 - R) * W[12]);
...
export function checkEaseHellRecovery(
  ...
  const shouldBoost = consecutivePasses >= config.consecutivePasses && currentEaseFactor < 2.5;
  if (!shouldBoost) {
