...
export class MarkdownExamParser {
  ...
  private fallbackParseSections(): Section[] {
    ...
    for (let i = 0; i < this.lines.length; i++) {
      const m = this.lines[i].match(
        /^#{1,6}\s+QUESTION\s+(\d+)[\s:]*(.*)?/i,
      );
      if (m) {
    ...
    if (questions.length > 0)
      return [{ id: "A", title: null, questions }];
    return [];
  }
  ...
  private parseParts(lines: string[], parentId: string): QuestionPart[] {
    ...
    while (i < lines.length) {
      ...
      if (!match)
        match = stripped.match(/^\s*[-*]\s+\(([a-z])\)\s*(.*)/);
      if (!match) {
      ...
      if (match) {
        ...
        while (j < lines.length) {
          ...
          if (/^\s*[-*]\s+\([a-z]\)/.test(ns)) {
            const subMatch = ns.match(
              /^\s*[-*]\s+\(([a-z])\)\s*(.*)/,
            );
            if (subMatch) {
              hasSubParts = true;
              const subText = subMatch[2].trim();
              let subMarks: number | string | null =
                this.extractMarks(subText);
              const subCleanText = subMarks !== null
                ? this.removeMarks(subText)
                : subText;
              subParts.push({
                id: `(${subMatch[1]})`,
          ...
          if (/^#{1,6}\s+/.test(ns)) {
            ...
            if (
              sm &&
              this.partLevel(sm[1]) <= this.partLevel(partId)
            )
              break;
          }
        ...
        const qType = this.determineType(
          partText,
          options,
          table,
        );
        const part: QuestionPart = {
          ...
          type: qType,
          marks,
          answerFormat: this.determineAnswerFormat(
            qType,
            options,
            table,
          ),
          content:
            contentBlocks.length > 0 ? contentBlocks : null,
          options: options.length > 0 ? options : null,
          table: table,
          sourceRefs:
            sourceRefs.length > 0 ? sourceRefs : null,
  ...
  private isStructural(
    ...
    if (m)
      return ["question", m[1], m[2]?.trim() || null];
  ...
  private determineType(
    ...
    if (
      /calculate|formula|compute|determine|gradient/.test(lower)
    )
      return "calculation";
    if (/draw|diagram|sketch/.test(lower)) return "diagram";
    if (/write code|program|delphi/.test(lower))
      return "programming";
