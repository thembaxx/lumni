...
export class MarkdownExamParser {
  ...
  private parseSections(): Section[] {
    const nonImageLines = this.lines.filter((l) => l.trim() && !l.trim().startsWith("!["));
        ...
        if (currentSection) {
          if (currentQuestion && questionStartLine !== null) {
            ...
            sectionQuestions.push(
              this.buildQuestion(currentQuestion, lines.slice(questionStartLine, lineIdx)),
            );
  ...
  private fallbackParseSections(): Section[] {
    ...
    for (let i = 0; i < this.lines.length; i++) {
      ...
      if (m) {
        ...
        questions.push(
          this.buildQuestion({ id: m[1], title: m[2]?.trim() || null }, this.lines.slice(i, end)),
        );
      }
    }
  ...
  private buildQuestion(qInfo: { id: string; title: string | null }, lines: string[]): Question {
        ...
        while (j < lines.length) {
          ...
          if (/^\s*[-*]\s+\([a-z]\)/.test(ns)) {
            ...
            if (subMatch) {
              ...
              const subMarks: number | string | null = this.extractMarks(subText);
              const subCleanText = subMarks !== null ? this.removeMarks(subText) : subText;
              subParts.push({
                id: `(${subMatch[1]})`,
  ...
  private extractCodeBlock(lines: string[], start: number): ContentBlock | null {
