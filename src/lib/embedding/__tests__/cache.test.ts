...
import { deleteEmbedding, getEmbedding, getEmbeddingsBySubject, storeEmbedding } from "../cache";
...
describe("embedding cache", () => {
  ...
  it("retrieves embeddings by subject", async () => {
    ...
    const math = await getEmbeddingsBySubject("mathematics", da.questionEmbeddings);
    expect(math.length).toBe(2);
    const phys = await getEmbeddingsBySubject("physical-sciences", da.questionEmbeddings);
