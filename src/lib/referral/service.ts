...
export async function getReferralCode(userId: string): Promise<ReferralCodeDoc | null> {
  const docs = await listDocuments<ReferralCodeDoc>(COLLECTIONS.REFERRAL_CODES, [
...
export async function createReferralCode(data: { userId: string; code: string }): Promise<string> {
...
export async function getReferralByCode(code: string): Promise<ReferralCodeDoc | null> {
  const docs = await listDocuments<ReferralCodeDoc>(COLLECTIONS.REFERRAL_CODES, [
...
export async function getReferralsByReferrer(referrerId: string): Promise<ReferralDoc[]> {
...
export async function getReferralCountThisMonth(referrerId: string): Promise<number> {
...
export async function getReferralByReferee(refereeId: string): Promise<ReferralDoc | null> {
  const docs = await listDocuments<ReferralDoc>(COLLECTIONS.REFERRALS, [
    Query.equal("refereeId", refereeId),
    Query.limit(1),
  ]);
...
export async function getReferrerCodeForUserId(userId: string): Promise<string | null> {
