...
const mockListDocuments = vi.fn<(collection: string, queries: string[]) => unknown[]>();
const mockCreateDocument = vi.fn<(collection: string, data: Record<string, unknown>) => string>();
const mockUpdateDocument =
  vi.fn<(collection: string, documentId: string, data: Record<string, unknown>) => void>();
