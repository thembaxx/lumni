...
import { __setDepsForTesting, getSavedWords, isWordSaved, removeWord, saveWord } from "../service";
...
describe("vocabulary service", () => {
  ...
  describe("saveWord", () => {
    ...
    it("lowercases the word", async () => {
      const result = await saveWord(userId, "Hello", "greeting", "en", "manual", "dictionary");
      expect(result?.word).toBe("hello");
    });
