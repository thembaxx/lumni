...
export async function listDocuments<T>(collection: string, queries: string[] = []): Promise<T[]> {
  ...
  const response = await db.listDocuments(APPWRITE_DATABASE_ID, collection, queries);
...
export async function getDocument<T>(collection: string, documentId: string): Promise<T | null> {
  ...
  try {
    const db = await getDb();
    const doc = await db.getDocument(APPWRITE_DATABASE_ID, collection, documentId);
...
export async function createDocument(
  ...
  const db = await getDb();
  const doc = await db.createDocument(APPWRITE_DATABASE_ID, collection, "unique()", data);
  return doc.$id;
}
