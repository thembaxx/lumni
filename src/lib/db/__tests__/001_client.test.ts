...
const mockDb = {
  listDocuments: vi.fn(async (_dbId: string, _coll: string, _queries: string[]) => ({
    documents: [],
  })),
  getDocument: vi.fn(async (_dbId: string, _coll: string, _docId: string) => ({})),
  createDocument: vi.fn(
    async (_dbId: string, _coll: string, _docId: string, _data: Record<string, unknown>) => ({
      $id: "doc-123",
    }),
  ),
  ...
  deleteDocument: vi.fn(async (_dbId: string, _coll: string, _docId: string) => {}),
};
...
describe("listDocuments", () => {
  ...
  test("passes database ID, collection, and queries to Appwrite", async () => {
    ...
    expect(mockDb.listDocuments).toHaveBeenCalledWith("test-db-id", "questions", [
      "limit(10)",
      "offset(0)",
    ]);
  });
...
describe("createDocument", () => {
  ...
  test("passes data with unique() ID", async () => {
    ...
    expect(mockDb.createDocument).toHaveBeenCalledWith("test-db-id", "topics", "unique()", {
      name: "Algebra",
      subjectId: "math",
    });
  });
...
describe("updateDocument", () => {
  test("calls Appwrite updateDocument with correct args", async () => {
    ...
    expect(mockDb.updateDocument).toHaveBeenCalledWith("test-db-id", "subjects", "doc-1", {
      name: "Updated",
    });
  });
});
...
describe("deleteDocument", () => {
  test("calls Appwrite deleteDocument with correct args", async () => {
    ...
    expect(mockDb.deleteDocument).toHaveBeenCalledWith("test-db-id", "subjects", "doc-1");
  });
});
