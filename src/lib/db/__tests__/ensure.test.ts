...
const mockDb = {
  ...
  listAttributes: async (_dbId: string, collId: string) => {
    ...
    if (listAttrsResult.attributes) return { attributes: listAttrsResult.attributes };
  ...
  createStringAttribute: async (_dbId: string, collId: string, name: string) => {
    createAttrCalls.push({ coll: collId, name });
    if (createAttrError) throw createAttrError;
    if (!collections[collId]) collections[collId] = { id: collId, attrs: {}, indexes: [] };
    collections[collId].attrs[name] = {};
  },
  createIntegerAttribute: async (_dbId: string, collId: string, name: string) => {
    createAttrCalls.push({ coll: collId, name });
    if (createAttrError) throw createAttrError;
    if (!collections[collId]) collections[collId] = { id: collId, attrs: {}, indexes: [] };
    collections[collId].attrs[name] = {};
  },
  createBooleanAttribute: async (_dbId: string, collId: string, name: string) => {
    createAttrCalls.push({ coll: collId, name });
    if (createAttrError) throw createAttrError;
    if (!collections[collId]) collections[collId] = { id: collId, attrs: {}, indexes: [] };
    collections[collId].attrs[name] = {};
  },
  createDatetimeAttribute: async (_dbId: string, collId: string, name: string) => {
    createAttrCalls.push({ coll: collId, name });
    if (createAttrError) throw createAttrError;
    if (!collections[collId]) collections[collId] = { id: collId, attrs: {}, indexes: [] };
    collections[collId].attrs[name] = {};
  ...
  listIndexes: async (_dbId: string, collId: string) => {
    ...
    if (listIndexesResult.indexes) return { indexes: listIndexesResult.indexes };
    const coll = collections[collId];
  ...
  listDocuments: async (_dbId: string, _collId: string, _queries: unknown[]) => {
    return { documents: [] as unknown[], total: 0 };
  },
  createDocument: async (_dbId: string, _collId: string, _docId: string, doc: unknown) => {
...
describe("ensureAppwrite", () => {
  ...
  test("reports success when db and collections exist", async () => {
    const report = await ensureAppwrite(undefined, mockDb as unknown as NodeDatabases);
