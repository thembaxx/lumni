...
describe("parseExamPaperFilename", () => {
  ...
  test("parses memo filename", () => {
    const r = parseExamPaperFilename("2024_physical-sciences_p2_memo.pdf") as NonNullable<
      ReturnType<typeof parseExamPaperFilename>
    >;
    expect(r).not.toBeNull();
    expect(r.year).toBe(2024);
  ...
  test("handles underscore-separated subject names", () => {
    const r = parseExamPaperFilename("2023_computer_applications_technology_p1.pdf") as NonNullable<
      ReturnType<typeof parseExamPaperFilename>
    >;
    expect(r).not.toBeNull();
