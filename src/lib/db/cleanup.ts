...
async function cleanupPage(cutoffIso: string, alreadyDeleted: number): Promise<CleanupPageResult> {
  const response = await databases.listDocuments(APPWRITE_DATABASE_ID, COLLECTIONS.QUESTIONS, [
