...
function makeCompetency(subjectId: string, topicId: string, score: number): CompetencyRecord {
  return {
    subjectId,
    topicId,
    bloomLevel: "apply",
...
describe("PathEngine", () => {
  ...
  describe("getNextTopics", () => {
    ...
    test("flags topics with unmet prerequisites as skip", async () => {
      const result = await engine.getNextTopics("mathematics", competencyMap([]));
    ...
    test("recommends study for topics with unmet prereqs as skip, ready as study", async () => {
      const result = await engine.getNextTopics("mathematics", competencyMap([["algebra", 90]]));
    ...
    test("sorts by priority: study < practice < review < skip", async () => {
      const result = await engine.getNextTopics("mathematics", competencyMap([["algebra", 70]]));
  ...
  describe("getNextAction", () => {
    ...
    test("returns null when all topics are skip", async () => {
      ...
      const result = await engine.getNextAction(["mathematics"], competencyMap(allMastered));
      expect(result).toBeNull();
    });

    test("returns the highest priority actionable topic", async () => {
      const result = await engine.getNextAction(["mathematics"], competencyMap([["algebra", 50]]));
    ...
    test("returns study for ready topics", async () => {
      const result = await engine.getNextAction(["mathematics"], competencyMap([["algebra", 90]]));
      expect(result).not.toBeNull();
  ...
  describe("generateStudyPlan", () => {
    ...
    test("fills daily sessions up to dailyGoalMinutes", async () => {
      ...
      const totalMinutes = plan[0].sessions.reduce((sum, s) => sum + s.minutes, 0);
