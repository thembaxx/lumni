...
export class StudyPlannerService {
  ...
  async generatePlan(settings?: {
    ...
    try {
      ...
      const studyDays = settings?.includeWeekends ? [0, 1, 2, 3, 4, 5, 6] : [1, 2, 3, 4, 5];
      ...
      const examDateInfos: ExamDateInfo[] = existingPlan.examDates.map((ed) => ({
        subjectId: ed.subject,
        date: new Date(ed.date).toISOString().split("T")[0],
      }));

      const service = getStudyPlannerService();
      const algorithmPlan = await service.generateStudyPlan(planSettings, examDateInfos);
      ...
      for (const ed of existingPlan.examDates) {
        ...
        if (!examSubjectsByDate.has(dateStr)) examSubjectsByDate.set(dateStr, new Set());
      ...
      for (const t of sortedTopics) {
        ...
        if (scheduledDate) {
          const isExamDay = examSubjectsByDate.get(scheduledDate)?.has(t.subjectId);
          ...
          newSessions.push({
            ...
            duration: enriched?.suggestedMinutes ?? Math.round(t.estimatedMinutes),
