...
describe("aiSolver.execute — RAG integration", () => {
  test("calls getSourceForQuestion with the question and userId", async () => {
    await aiSolver.execute({ question: "When is the 2026 Maths Paper 2 exam?" }, "user-1", deps);
  ...
  test("injects XML into user prompt when sources are found", async () => {
    ...
    const [, userPrompt] = mockGenerateWithSystem.mock.calls[0] as [string, string, unknown];
  ...
  test("appends buildPromptInstruction to system prompt when sources are found", async () => {
    ...
    const [systemPrompt] = mockGenerateWithSystem.mock.calls[0] as [string, string, unknown];
  ...
  test("returns empty sources when web context is empty", async () => {
    const result = await aiSolver.execute({ question: "2 + 2" }, "user-1", deps);

    expect(result.sources).toEqual([]);
  });

  test("passes userId=undefined when none provided", async () => {
    await aiSolver.execute({ question: "When is the 2026 Maths exam?" }, undefined, deps);
