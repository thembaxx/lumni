...
const { convertPdfWithMarker, uploadImagesAndRewriteMarkdown } = await import("../marker-client");
...
describe("convertPdfWithMarker", () => {
  ...
  test("sends POST request to /convert endpoint", async () => {
    const fetchMock = vi.fn(async (url: string | URL, opts?: RequestInit): Promise<Response> => {
  ...
  test("throws on non-ok response", async () => {
    ...
    await expect(convertPdfWithMarker(Buffer.from("test"), "bad.pdf")).rejects.toThrow(
      "Marker conversion failed (400)",
    );
  });
});
...
describe("uploadImagesAndRewriteMarkdown", () => {
  ...
  test("throws when upload fails", async () => {
    ...
    await expect(
      uploadImagesAndRewriteMarkdown("![img](x.png)", [{ filename: "x.png", data: "base64,fail" }]),
