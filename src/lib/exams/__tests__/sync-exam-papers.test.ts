...
const { mockExistsSync, mockReadFileSync, mockReaddirSync, mockWriteFileSync } = vi.hoisted(() => ({
...
const { mockListDocuments, mockGetDocument, mockCreateDocument, mockUpdateDocument } = vi.hoisted(
  () => ({
    mockListDocuments: vi.fn(async (_dbId: string, _collection: string, _queries?: string[]) => ({
      documents: [],
      total: 0,
    })),
    mockGetDocument: vi.fn(async (_dbId: string, _collection: string, _docId: string) => null),
    mockCreateDocument: vi.fn(
      async (_dbId: string, _collection: string, _docId: string, _data: unknown) =>
        ({ $id: "new-id" }) as unknown,
    ),
    mockUpdateDocument: vi.fn(
      async (_dbId: string, _collection: string, _docId: string, _data: unknown) => null,
    ),
  }),
);
...
const { ensureExamPapersSynced, isSyncCompleted, syncExamPapers, forceSyncExamPapers } =
