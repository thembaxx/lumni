...
export class PlatformAnalyticsService {
  async fetchAnalytics(): Promise<PlatformAnalytics> {
    ...
    const subjectPopularity: Array<{ subject: string; code: string }> = (
    ...
    const recentSessionDocsTyped = recentSessionDocs as Array<Record<string, unknown>>;
    const completedSessions = recentSessionDocsTyped.filter(
      (s: Record<string, unknown>) => s.endedAt || s.startedAt,
    );
    ...
    const totalCorrect = completedSessions.reduce(
      (sum: number, s: Record<string, unknown>) => sum + ((s.correctCount as number) || 0),
      0,
    );
    const totalAnswered = completedSessions.reduce(
      (sum: number, s: Record<string, unknown>) => sum + ((s.questionsAnswered as number) || 0),
      0,
    );
    ...
    return {
      ...
      overallAccuracy,
      subjectPopularity: (
        subjectSessionCounts as Array<{ subject: string; code: string; sessions: number }>
