...
function tokenize(expression: string): Token[] {
  ...
  while (i < expression.length) {
    ...
    if (ch === "-") {
      ...
      const isUnary = !prev || prev.type === "op" || prev.type === "lp" || prev.type === "fact";
      tokens.push({
        type: "op",
        op: isUnary ? "u-" : "-",
        prec: isUnary ? 5 : 2,
    ...
    if ((ch >= "a" && ch <= "z") || (ch >= "A" && ch <= "Z") || ch === "_") {
      let name = "";
      while (i < expression.length && /[\w.]/.test(expression[i])) name += expression[i++];
...
export function safeEval(expr: string, _angleMode: AngleMode): number {
  ...
  if (result.pos !== tokens.length) throw new Error("Unexpected tokens after expression");
  return result.val;
}
