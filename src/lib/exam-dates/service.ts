...
export async function getExamDates(session: string, year: number): Promise<ExamSlot[]> {
  ...
  try {
    const cached = await _deps.db.examDates.where("cacheKey").equals(key).first();
    if (cached) {
...
export async function getNextExams(session: string, year: number, count = 2): Promise<ExamSlot[]> {
