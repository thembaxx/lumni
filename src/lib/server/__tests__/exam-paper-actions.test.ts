...
const {
  ...
  mockDeleteDocument: vi.fn(async (_dbId: string, _collection: string, _docId: string) => null),
}));
...
describe("uploadExamPaper", () => {
  test("throws when user not authenticated", async () => {
    mockUserId.val = null;
    await expect(uploadExamPaper({ year: 2024, paperNumber: 1, type: "paper" })).rejects.toThrow(
...
describe("deleteExamPaper", () => {
  ...
  test("throws when exam paper not found", async () => {
    ...
    await expect(deleteExamPaper("nonexistent")).rejects.toThrow("Document not found");
  });
