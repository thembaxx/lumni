...
vi.mock("@/lib/db/client", () => ({
  ...
  createDocument: vi.fn(
    async (_collection: string, _data: Record<string, unknown>) => mockCreateDocumentResult.val,
  ),
  ...
  updateDocument: vi.fn(
    async (collection: string, documentId: string, data: Record<string, unknown>) => {
...
describe("fetchUserProgress", () => {
  ...
  test("handles null progress with existing sessions", async () => {
    mockListDocumentsResults.val.user_progress = [];
    mockListDocumentsResults.val.study_sessions = [{ questionsAnswered: 20, correctCount: 15 }];
...
describe("adminUploadExamPaper", () => {
  test("returns auth error when not authenticated", async () => {
    ...
    await expect(adminUploadExamPaper(formData)).rejects.toThrow("Authentication required");
  });
