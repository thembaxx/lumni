...
export async function uploadExamPaper(options: UploadExamPaperOptions): Promise<ExamPaperRecord> {
  ...
  const existing = await databases.listDocuments(APPWRITE_DATABASE_ID, COLLECTIONS.EXAM_PAPERS, [
  ...
  if (type === "memo") {
    const paperDocs = await databases.listDocuments(APPWRITE_DATABASE_ID, COLLECTIONS.EXAM_PAPERS, [
      Query.equal("subjectCode", subjectCode),
      Query.equal("year", year),
      Query.equal("paperNumber", paperNumber),
  ...
  await databases.createDocument(APPWRITE_DATABASE_ID, COLLECTIONS.EXAM_PAPERS, id, docData);
