...
export function useImageChatWithSend(
  ...
  const sendImage = useCallback(
    async (file: File) => {
      ...
      chat.setMessages((prev) => [...prev, userMessage as ChatMessage] as ChatMessage[]);
      ...
      try {
        ...
        chat.setMessages(
          (prev) =>
            [
              ...prev.map((m) =>
                m.id === userMessage.id ? { ...m, processingStatus: "success" as const } : m,
              ),
              assistantMessage,
            ] as ChatMessage[],
        );
      ...
      try {
        const response = await fetch("/api/chat/image", {
          ...
          body: JSON.stringify({
            ...
            imageName: chat.messages.find((m) => m.id === messageId)?.imageFileName,
          }),
        });
        ...
        chat.setMessages(
          (prev) =>
            prev.map((m) =>
              m.id === messageId ? { ...m, processingStatus: "success" as const } : m,
            ) as ChatMessage[],
        );
