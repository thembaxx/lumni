...
function gradeAnswer(question: Question, answer: UserAnswer): Promise<GradingResult> {
...
export function useQuestionEngine(params?: GenerationParams, options?: UseQuestionEngineOptions) {
  ...
  const gradeMutation = useMutation({
    ...
    onSuccess: () => queryClient.invalidateQueries({ queryKey: ["questionEngine"] }),
  });
  ...
  const grade = useCallback(
    (question: Question, answer: UserAnswer) => gradeMutation.mutateAsync({ question, answer }),
    [gradeMutation],
  );
