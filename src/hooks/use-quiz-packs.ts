...
export function useQuizPacks() {
  ...
  useEffect(() => {
    quizPackService.getStorageUsage().then((usage) => setStorageBytes(usage.usedBytes));
  }, []);

  const generate = useCallback(async (subject: string, topic: string | null, count: number) => {
    setGenerating(true);
    try {
  ...
  const storagePercentage = Math.min((storageBytes / MAX_PACK_STORAGE_BYTES) * 100, 100);
