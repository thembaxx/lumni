...
export function useEnrolledSubjects() {
  ...
  const migratePrefsSubjects = useCallback(async () => {
    ...
    const subjectIds = currentPrefs.flatMap((name) => {
      const sub = subjectsData.find((s) => s.name.toLowerCase() === name.toLowerCase());
  ...
  const subjects = useMemo(() => data?.subjects ?? (subjectsData as Subject[]), [data?.subjects]);
  ...
  return {
    ...
    isEnrolled: useCallback((subjectId: string) => selectedIds.includes(subjectId), [selectedIds]),
  };
}
...
export function useFilteredSubjects(searchQuery: string, enrolledOnly = false) {
  ...
  const source = enrolledOnly ? enrolledSubjects : (data?.subjects ?? (subjectsData as Subject[]));
