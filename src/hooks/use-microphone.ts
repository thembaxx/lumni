...
export function useMicrophone(refs: MicrophoneRefs, values: MicrophoneValues) {
  const { streamRef, audioContextRef, animationRef, analyserRef, historyRef } = refs;
  const { active, deviceId, fftSize, smoothingTimeConstant, onError, onStreamReady, onStreamEnd } =
    values;
