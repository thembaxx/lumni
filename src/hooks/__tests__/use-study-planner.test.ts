...
vi.mock("@/lib/utils/study-planner", () => ({
  loadStudyPlanFromDexie: () => Promise.resolve({ sessions: [], examDates: [], generatedAt: 0 }),
  loadStudyPlan: () => ({
  ...
  autoScheduleSessions: (_subjects: string[], _weakTopics: Record<string, string[]>) => {
    return {
      sessions: [...mockPlan.sessions],
      examDates: [...mockPlan.examDates],
      generatedAt: mockPlan.generatedAt,
...
describe("useStudyPlanner operations", () => {
  ...
  test("removeSession deletes a session and refreshes", () => {
    ...
    expect(result.current.plan.sessions.find((s) => s.id === "session-to-remove")).toBeUndefined();
  });
  ...
  test("removeExam deletes an exam and refreshes", () => {
    ...
    expect(result.current.plan.examDates.find((e) => e.id === "exam-to-remove")).toBeUndefined();
  });
  ...
  test("generatePlan dynamically imports study-planner-service", async () => {
    ...
    const remaining = result.current.plan.sessions.filter((s) => s.id === "existing-review");
