...
interface ExamSessionState {
  ...
  initSession: (paper: ExamPaper, paperId: string, durationMinutes: number) => void;
      ...
      if (record) {
        return {
          state: {
            ...
            sessionId: null,
            answers: JSON.parse(record.answers || "{}") as Record<string, ExamAnswer>,
  ...
  persist(
    (set, get) => ({
      ...
      toggleFlag: (partId) => {
        const { flags } = get();
        set({
          flags: flags.includes(partId) ? flags.filter((f) => f !== partId) : [...flags, partId],
        });
      },
...
function validateHydratedState(state: unknown): boolean {
  ...
  return typeof s.paperId === "string" || s.paperId === null || s.paperId === undefined;
}
