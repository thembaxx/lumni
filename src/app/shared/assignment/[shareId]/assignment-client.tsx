  ...
  useEffect(() => {
    ...
    (async () => {
      try {
        ...
        if (shared) {
          const parsed = JSON.parse(shared.question as string) as AssignmentData;
          setData(parsed);
        }
      } catch (err) {
  ...
  if (!data) {
    return (
      <PageContainer className="min-h-dvh items-center justify-center gap-4 py-8 text-center">
        ...
        <h1 className="font-bold font-heading text-2xl">Assignment Not Found</h1>
  ...
  return (
    <PageContainer className="min-h-dvh gap-6">
      <div className="flex flex-col gap-2">
        ...
        <p className="text-muted-foreground text-sm">This assignment was shared by your teacher</p>
      </div>

      <div className="grid grid-cols-2 gap-4">
        <div className="rounded-xl border bg-card p-4">
          <p className="text-muted-foreground text-xs uppercase tracking-wide">Questions</p>
