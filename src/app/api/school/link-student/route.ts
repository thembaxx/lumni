...
export const POST = createRouteHandler({
  ...
  execute: async ({ body, userId }) => {
    ...
    const member = await addSchoolMember(schoolId, userId, "student");
    if (!member) throw new HttpError(500, "Failed to link student to school");
