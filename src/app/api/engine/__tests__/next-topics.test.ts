...
const mockGetNextTopics = vi.fn<(subject: string, competencyMap: unknown) => unknown>();
const mockListDocuments = vi.fn<(collection: string, queries: unknown[]) => unknown>();
...
describe("GET /api/engine/next-topics", () => {
  ...
  test("returns recommendations and summary from pathEngine", async () => {
    ...
    mockGetNextTopics.mockResolvedValue([{ topicId: "calculus", reason: "Next logical step" }]);

    const req = new NextRequest("http://localhost/api/engine/next-topics?subject=mathematics");

    const res = await GET(req);
    const body = await res.json();

    expect(res.status).toBe(200);
    expect(body.recommendations).toEqual([{ topicId: "calculus", reason: "Next logical step" }]);
  ...
  test("computes correct summary stats from competencies", async () => {
    ...
    const req = new NextRequest("http://localhost/api/engine/next-topics?subject=math");
  ...
  test("handles empty competencies gracefully", async () => {
    ...
    const req = new NextRequest("http://localhost/api/engine/next-topics?subject=physics");
