...
export const GET = withRateLimit(
  createRouteHandler({
    ...
    execute: async ({ req }) => {
      ...
      const docs = await listDocuments<Record<string, unknown>>(COLLECTIONS.COMPETENCIES, [
      ...
      const developing = competencies.filter((c) => c.level === "developing").length;
      const proficient = competencies.filter((c) => c.level === "proficient").length;
      const mastered = competencies.filter((c) => c.level === "mastered").length;
      const averageScore =
        total > 0
          ? Math.round((competencies.reduce((s, c) => s + c.score, 0) / total) * 100) / 100
          : 0;
