...
import { createPost, getGroupMembers, getGroupPosts } from "@/lib/study-groups/service";
...
export const GET = createRouteHandler({
  ...
  execute: async ({ userId, params }) => {
    ...
    const isMember = membersResult.success && membersResult.data.some((m) => m.userId === userId);
    if (!isMember) {
...
export const POST = createRouteHandler({
  ...
  execute: async ({ userId, params, req }) => {
    ...
    const [body, userName] = await Promise.all([req.json(), getAuthenticatedUserName()]);
