...
export const POST = createRouteHandler<EnrollBody>({
  ...
  execute: async ({ body, userId }) => {
    ...
    const existing = await listDocuments<{ $id: string }>(COLLECTIONS.USER_SUBJECTS, [
      Query.equal("userId", userId),
    ]);

    await Promise.all(existing.map((doc) => deleteDocument(COLLECTIONS.USER_SUBJECTS, doc.$id)));
