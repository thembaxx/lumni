...
async function fetchParsedPaper(id: string): Promise<ExamPaper | null> {
  try {
    ...
    const fileKeys: Record<string, string> = fileKeysRaw ? JSON.parse(fileKeysRaw) : {};
...
export const POST = createRouteHandler({
  ...
  execute: async ({ params }) => {
    ...
    const { questions, stats } = extractQuestionsFromPaper(paper, memo, subject, year, paperNumber);
