...
export const GET = createRouteHandler({
  ...
  execute: async ({ userId, params }) => {
    ...
    const doc = await databases.getDocument(APPWRITE_DATABASE_ID, COLLECTIONS.EXAM_SESSIONS, id);
...
export const DELETE = createRouteHandler({
  ...
  execute: async ({ userId, params }) => {
    ...
    await databases.deleteDocument(APPWRITE_DATABASE_ID, COLLECTIONS.EXAM_SESSIONS, id);
