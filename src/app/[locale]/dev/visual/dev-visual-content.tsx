...
function devVisualReducer(state: DevVisualState, action: DevVisualAction): DevVisualState {
  switch (action.type) {
    case "SET_SUBJECT":
...
export default function DevVisualPage() {
  ...
  const questionText = "Solve for x in the equation $x^2 - 5x + 6 = 0$. Show the parabola graph.";
  ...
  return (
    <div className="mx-auto flex min-h-dvh max-w-4xl flex-col gap-4 bg-background p-4 pb-20">
      <h1 className="font-heading font-semibold text-2xl">Visual Engine Test</h1>
      ...
      <div className="overflow-hidden rounded-card-lg bg-card shadow-level-2">
        <div className="flex flex-col gap-3 p-4">
          <div className="grid grid-cols-1 gap-2 sm:grid-cols-3">
            <Select
              value={state.subject}
              onValueChange={(v) => v && dispatch({ type: "SET_SUBJECT", payload: v })}
            >
              <SelectTrigger>
                <SelectValue />
              </SelectTrigger>
            ...
            />
            <Button onClick={handleResolve} disabled={state.isLoading} variant="default">
