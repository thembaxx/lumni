...
function ErrorTypeSelect({ value, onChange }: { value: string; onChange: (v: ErrorType) => void }) {
  return (
...
export function ReviewClient() {
  const { getWrongAnswers, markReviewed, clearReviewed, updateErrorType } = useWrongAnswerJournal();
  ...
  const { data: entries = [], isLoading: loading } = useQuery({
    ...
    queryFn: () => getWrongAnswers(filterSubject || undefined, filterTopic || undefined),
  });
        ...
        {entries.length === 0 ? (
          ...
          <div className="flex flex-col gap-3">
            <p className="text-muted-foreground text-sm">
              {entries.length} question{entries.length !== 1 ? "s" : ""} to review
            </p>
            {entries.map((entry) => (
              <div
                key={entry.id}
              ...
              >
                <div className="flex flex-col gap-2 p-4 pb-2">
                  <div className="flex flex-wrap items-center gap-2">
                    ...
                    <ErrorTypeSelect
                      value={errorTypes[entry.id ?? 0] ?? entry.errorType ?? "unknown"}
                ...
                <div className="flex flex-col gap-3 p-4 pt-0">
                  <div className="grid grid-cols-2 gap-3">
                    <div className="rounded-lg bg-destructive/5 p-3">
                      <p className="mb-1 font-medium text-destructive text-xs">Your answer</p>
                      <p className="overflow-wrap-anywhere text-sm">{entry.userAnswer}</p>
                    </div>
                    <div className="rounded-lg bg-success/5 p-3">
                      <p className="mb-1 font-medium text-success text-xs">Correct answer</p>
                      <p className="overflow-wrap-anywhere text-sm">{entry.correctAnswer}</p>
                    </div>
                  </div>
                  {entry.explanation && (
