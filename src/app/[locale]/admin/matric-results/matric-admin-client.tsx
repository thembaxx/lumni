...
export function MatricAdminClient() {
  ...
  const parsePreview = useCallback((text: string) => {
    ...
    const required = ["candidatenumber", "firstname", "lastname", "examyear", "session", "subject", "mark", "outof", "level"];
  ...
  const handleUpload = useCallback(async () => {
    ...
    try {
      ...
      const res = await fetch("/api/admin/matric-results/upload", { method: "POST", body: formData });
  ...
  return (
    <div className="mx-auto flex min-h-dvh max-w-4xl flex-col gap-6 bg-background p-6">
      <div className="min-w-0">
        <h1 className="font-heading font-semibold text-2xl tracking-tight">Matric Results Upload</h1>
        <p className="mt-1 text-muted-foreground text-sm">
          Upload CSV data with NSC/SC matric results. Columns: candidateNumber, firstName, lastName, examYear, session, subject, mark, outOf, level
        </p>
      </div>
      ...
      {result && (
        <div className={`${CARD_CLASS} p-4`}>
          <p className="font-medium">
            {result.inserted > 0
              ? `Inserted ${result.inserted} results`
              : "No results inserted"}
          </p>
          {result.errors > 0 && (
