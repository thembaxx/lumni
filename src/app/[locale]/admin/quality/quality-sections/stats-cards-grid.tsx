  ...
  return (
    <div className="overflow-hidden rounded-card-lg border border-border/80 bg-card shadow-level-2 transition-colors">
      <header className="rounded-t-card-lg border-border/80 border-t p-4 pb-2">
        <h2 className="font-medium font-sans text-muted-foreground text-sm">{label}</h2>
      </header>
...
export function StatsCardsGrid({
  ...
  return (
    <div className="grid grid-cols-2 gap-4 lg:grid-cols-4">
      <StatCard label="Total Requests" value={String(totalRequests)} valueColor="" />
      <StatCard
        label="Success Rate"
        value={`${successRate}%`}
        valueColor={
      ...
      <StatCard
        ...
        valueColor={
          avgScore >= 80 ? "text-success" : avgScore >= 50 ? "text-warning" : "text-destructive"
        }
      />
      <StatCard
